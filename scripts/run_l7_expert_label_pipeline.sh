...
NPROC="$TRAIN_GPUS" LOAD_CONFIG=l7_expert TAG="$TAG" RUN_ID="$RUN_ID" \
  OUTPUT_ROOT="$OUTPUT_ROOT" WANDB_MODE="${WANDB_MODE:-online}" \
  bash scripts/run_l7_training_pipeline.sh "task.num_envs=$TRAIN_NUM_ENVS" "$@"
