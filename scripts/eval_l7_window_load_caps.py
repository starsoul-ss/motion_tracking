...
def main():
    ...
    def assign_motion(env_id: int):
        ...
        cursor += 1
        states[env_id] = State(active=True, motion_id=motion_id, load_kg=args.max_load_kg)
    ...
    def record(state: State, status: str, load_kg: float):
        start, end = windows[state.motion_id][state.window_idx]
    ...
    def queue_reset(env_id: int, state: State):
            ...
            for env_id, state in enumerate(states):
                ...
                if success:
                    ...
                    if state.confirming:
                        ...
                        queue_reset(env_id, state)
                        continue
                    record(state, "success", state.load_kg)
                    state.window_idx += 1
                    if state.window_idx >= len(windows[state.motion_id]):
                        if assign_motion(env_id):
                            queue_reset(env_id, states[env_id])
